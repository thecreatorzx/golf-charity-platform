...
import { getPublishedDraws } from '../../api/services';
            ...
            <div className="space-y-2">
              ...
              {draws.map((d) => (
                <button key={d.id} onClick={() => setSelected(d)}
                  className={`w-full text-left p-4 rounded-xl border transition-all ${selected?.id === d.id ? 'bg-[#C8F04D]/10 border-[#C8F04D]/30 text-[#C8F04D]' : 'border-white/8 text-white/60 hover:border-white/15 hover:text-white/80 bg-white/[0.02]'}`}>
                  <div className="font-bold text-sm">{MONTHS[d.month - 1]} {d.year}</div>
                  <div className="text-xs mt-0.5 opacity-60">
                    {d.winners?.length || 0} winners · ₹{d.prizePool?.toLocaleString()}
                  </div>
                </button>
              ))}
            </div>
                  ...
                  <Card className="p-6" glow>
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-6">
                      {[
                        ...
                        { label: 'Winners', value: selected.winners?.length || 0 },
