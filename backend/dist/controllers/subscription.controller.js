...
exports.mockActivate = exports.cancel = exports.fetchSubscription = exports.confirmSubscription = exports.initiateSubscription = void 0;
...
const cancel = async (req, res) => {
    try {
        const subscription = await (0, subscription_service_1.cancelSubscription)(req.userId);
...
const mockActivate = async (req, res) => {
    try {
        const { plan } = req.body;
        if (!plan || !['MONTHLY', 'YEARLY'].includes(plan)) {
            res.status(400).json({ message: 'Invalid plan' });
        ...
        const subscription = await (0, subscription_service_1.mockActivateSubscription)(req.userId, plan);
        res.json({ subscription });
    }
    catch {
        res.status(500).json({ message: 'Server error' });
